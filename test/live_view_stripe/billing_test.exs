defmodule LiveViewStripe.BillingTest do
  ...
  describe "subscriptions" do
    ...
    test "create_subscription/1 with valid data creates a subscription" do
      valid_attrs = %{cancel_at: ~N[2022-02-24 21:26:00], current_period_end_at: ~N[2022-02-24 21:26:00], status: "some status", stripe_id: "some stripe_id"}

      assert {:ok, %Subscription{} = subscription} = Billing.create_subscription(valid_attrs)
      assert subscription.cancel_at == ~N[2022-02-24 21:26:00]
      assert subscription.current_period_end_at == ~N[2022-02-24 21:26:00]
    ...
    test "update_subscription/2 with valid data updates the subscription" do
      subscription = subscription_fixture()
      update_attrs = %{cancel_at: ~N[2022-02-25 21:26:00], current_period_end_at: ~N[2022-02-25 21:26:00], status: "some updated status", stripe_id: "some updated stripe_id"}

      assert {:ok, %Subscription{} = subscription} = Billing.update_subscription(subscription, update_attrs)
      assert subscription.cancel_at == ~N[2022-02-25 21:26:00]
      assert subscription.current_period_end_at == ~N[2022-02-25 21:26:00]
